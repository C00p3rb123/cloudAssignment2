...
import { hashPassword, getS3, setS3, setRedis, getData  } from "../utils/utils";
...
router.get("/:platform", async (req, res) => {
  ...
  const userEmail = req["user"]?.email;
  const data = await getData(userEmail, redisClient);
  const requestedPlatform: ServiceStored= data.filter((service) => (service.platform === platform))[0];
  const returnedPlatform : ServiceRequest = {
    ...requestedPlatform,
    password: decryptPassword(requestedPlatform.password)
  }   
  await redisClient.quit();
  res.status(200).json({
